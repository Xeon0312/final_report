...
raw_data <- read_csv("./input/AAs3ddGb.csv")
dict <- read_lines("./input/gss_dict-1.txt", skip = 18) # skip is because of preamble content
# Now we need the labels because these are the actual responses that we need
labels_raw <- read_file("./input/gss_labels-1.txt")
...
ow = c("Don't know","NA")
gss <- gss %>% 
  mutate(citizenship = case_when(citizenship_status =="By birth" ~ "Canadian citizen",
                                 citizenship_status =="By naturalization" ~ "Permanent resident",
                                 citizenship_status %in% ow~"Other"
  ))
